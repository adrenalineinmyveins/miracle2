...
typedef struct sockaddr DataType; // 指明地址信息
...
LinkNode *creat_empty_linklist()  //创建一个空链表，用来记录用户
{
	LinkNode *head = NULL;
...
{
	LinkNode *temp = NULL;								
	temp = (LinkNode *)malloc(sizeof(LinkNode));//创建一个新链表节点

	(head->n)++;								//链表长度加一  
	strcpy(temp->pname,msg.name);             //存储用户名

	temp->data = data;							//IP  PORT 等地址信息
...
int main(int argc,const char *argv[])
{
	...
	if(sock_fd < 0)    
	{
		printf("fail to soc_fd:%s",strerror(errno));
		exit(EXIT_FAILURE);
	}
	...
	{
		send_message(sock_fd,server_in);     //本机的IP  通过
	}
	exit(EXIT_SUCCESS);
}
